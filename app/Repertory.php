...
class Repertory extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['id', 'name', 'address','creater_id','status','created_at','updated_at'];
    ...
    public function hasManyInventory() {
        ...
        return $this->hasMany('App\Inventory', 'repertory_id', 'id');
    }

    // 定义InventoryDetail表与Repertory表一对多关系
    public function hasManyInventoryDetail() {

        return $this->hasMany('App\InventoryDetail', 'repertory_id', 'id');
    }

    // 定义User表与Repertory表一对多关系
    public function hasManyUser() {

        return $this->hasMany('App\User', 'repertory_id', 'id');
    }
